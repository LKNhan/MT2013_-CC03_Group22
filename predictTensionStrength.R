...
MLR1 = lm(formula = tension_strenght ~ layer_height + wall_thickness + infill_density +
...
MLR2 = lm(formula = tension_strenght ~ layer_height + wall_thickness + infill_density +
            infill_pattern + nozzle_temperature + bed_temperature + print_speed + 
...
summary(MLR1)
summary(MLR2)
anova(MLR1, MLR2) #compare model 1 to model 2, no difference


#For chapter 6
#remove variables with P(r) > 0.05: infill_pattern, print speed
MLR3 = lm(formula = tension_strenght ~ layer_height + wall_thickness + infill_density +
